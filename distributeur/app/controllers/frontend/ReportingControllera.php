...
class ReportingController extends \app\core\FrontendController
{
    ...
    {
        $data['lang'] =  $this->lang->getLangFile($this->session->getAttribut('lang'));
        $paramsview = array('view' => sprintf('frontend/reporting/transactionjr'));
        $this->view($paramsview, $data);
    }
    ...
    public function processingUser()
    {
        ...
        if($obj->getAdmin() == 1 || $obj->getType_profil() == 1)
        {
            $next.=", region";
            $where.=" AND agence.province = region.idregion";
        }
        elseif($obj->getType_profil() == 2)
        {
            $next.=", region";
            $where.=" AND agence.province = region.idregion AND transaction.fk_agence=".$obj->getFk_agence();
        }
        elseif($obj->getType_profil() == 4)
        {
            $where.=" AND transaction.fk_agence=".$obj->getFk_agence();
        }
        else{
    ...
    public function  transactionJour($date1, $date2)
    {
        ...
        else if($obj->getType_profil() == 2)
        {
            $next.=", region";
            $where.=" AND agence.province = region.idregion AND transaction.fk_agence=".$obj->getFk_agence();
        }
        else if($obj->getType_profil() == 4)
        {
            $where.=" AND transaction.fk_agence=".$obj->getFk_agence();
        }
        else{

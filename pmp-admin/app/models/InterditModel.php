...
class InterditModel extends \app\core\BaseModel
{
    ///////////////////////////////////////************************************/////////////////////////////////
    //                                                                                                        //
  ...
  AND  service.rowid=:serviceID";
    ...
    public function getHistoriqueListInterdit($requestData = null)
    {
        $data = null;
        $sql = "SELECT rowid, nom , prenom , wholename , nationalite , date_nais , statut FROM interdits";
        ...
        /*if (is_array($requestData)) {
            ...
            if ($_REQUEST['search']['value']!="") {
                $sql .= " WHERE ( nom LIKE '%?%' ";
                $sql .= " OR prenom LIKE '%?%' ";
                $sql .= " OR wholename LIKE '%?%' ";
                $sql .= " OR nationalite LIKE '%?%' ";
                $sql .= " OR date_nais LIKE '%?%' ";
                $sql .= " OR statut LIKE '%?%' ) ";
                $data = [$requestData, $requestData, $requestData, $requestData, $requestData, $requestData];
            }
            $tabCol = ["nom" , "prenom" , "wholename" , "nationalite" , "date_nais" , "statut"];
            if (intval($_REQUEST['order'][0]['column']) < count($tabCol))
                $sql .= " ORDER BY " . $tabCol[$_REQUEST['order'][0]['column']] . " " . strtoupper($_REQUEST['order'][0]['dir']);
            $sql .= " LIMIT " . $_REQUEST['start'] . " ," . $_REQUEST['length'];
        }*/
        ...
        if ($_REQUEST['search']['value']!="") {
            $sql .= " WHERE ( nom LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR prenom LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR wholename LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR nationalite LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR date_nais LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR statut LIKE '%".$_REQUEST['search']['value']."%' ) ";
            //$data = [$requestData, $requestData, $requestData, $requestData, $requestData, $requestData];
        }
        $tabCol = ["nom" , "prenom" , "wholename" , "nationalite" , "date_nais" , "statut"];
        if (intval($_REQUEST['order'][0]['column']) < count($tabCol))
        ...
        for ($i = 0; $i < count($result); $i++) {
            $result[$i]['date_nais'] = (new \app\core\Utils())->date_fr4($result[$i]['date_nais']);
        }
        return $result;
    }
    public function getHistoriqueListInterditCount()
    {
        try {
            $sql = "SELECT rowid, nom , prenom , wholename , nationalite , date_nais , statut FROM interdits";
            if ($_REQUEST['search']['value']!="") {
    ...
    public function getHistoriqueListInterditInstance($requestData = null)
    {
        $data = null;
        $sql = "SELECT rowid, nom_complet, date_nais , date_interdit FROM alerts WHERE confirmer = 2";



        if ($_REQUEST['search']['value']!="") {
            $sql .= " AND ( nom_complet LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR date_nais LIKE '%".$_REQUEST['search']['value']."%%' ";
            $sql .= " OR date_interdit LIKE '%".$_REQUEST['search']['value']."%%' ) ";
        }
        $tabCol = ["nom_complet" , "date_nais" , "date_interdit"];
        if (intval($_REQUEST['order'][0]['column']) < count($tabCol))
            $sql .= " ORDER BY " . $tabCol[$_REQUEST['order'][0]['column']] . " " . strtoupper($_REQUEST['order'][0]['dir']);
        $sql .= " LIMIT " . $_REQUEST['start'] . " ," . $_REQUEST['length'];
        ...
        $utils = new \app\core\Utils();
        for ($i = 0; $i < count($result); $i++) {
            $result[$i]['date_nais'] = $utils->date_fr4($result[$i]['date_nais']);
            $result[$i]['date_interdit'] = $utils->date_fr4($result[$i]['date_interdit']);
        }
        return $result;
    }
    public function getHistoriqueListInterditInstanceCount($requestData=null)
    {
    ...
    public function getHistoriqueListInterditConfirme($requestData = null)
    {
        $data = null;
        $sql = "SELECT a.rowid, a.nom_complet, a.date_nais, CONCAT(u.prenom, ' ',u.nom), a.date_confirmation FROM alerts a INNER JOIN user u ON u.rowid = a.user_confirmation WHERE a.confirmer = 1";


        if ($_REQUEST['search']['value']!="") {
            $sql .= " AND ( a.nom_complet LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR a.date_nais LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR u.prenom LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR u.nom LIKE '%".$_REQUEST['search']['value']."%' ";
            $sql .= " OR a.date_confirmation LIKE '%".$_REQUEST['search']['value']."%' ) ";
            //$data = [$requestData, $requestData, $requestData];
        }
        $tabCol = ["a.nom_complet" , "a.date_nais" , "u.prenom" , "u.nom", "a.date_interdit"];
        if (intval($_REQUEST['order'][0]['column']) < count($tabCol))
            $sql .= " ORDER BY " . $tabCol[$_REQUEST['order'][0]['column']] . " " . strtoupper($_REQUEST['order'][0]['dir']);
        $sql .= " LIMIT " . $_REQUEST['start'] . " ," . $_REQUEST['length'];
        ...
        for ($i = 0; $i < count($result); $i++) {
            $result[$i]['date_nais'] = $utils->date_fr4($result[$i]['date_nais']);
            $result[$i]['date_confirmation'] = $utils->date_fr4($result[$i]['date_confirmation']);
        }
        return $result;
    }
    public function getHistoriqueListInterditConfirmeCount()
    {
        try {
            $sql = "SELECT a.rowid, a.nom_complet, a.date_nais, CONCAT(u.prenom, ' ',u.nom), a.date_confirmation FROM alerts a INNER JOIN user u ON u.rowid = a.user_confirmation WHERE a.confirmer = 1";
            if ($_REQUEST['search']['value']!="") {
    ...
    public function getHistoriqueListInterditLevee($requestData = null)
    {
        $data = null;
        $sql = "SELECT a.rowid, a.nom_complet, a.date_nais, CONCAT(u.prenom, ' ',u.nom), a.date_confirmation FROM alerts a INNER JOIN user u ON u.rowid = a.user_confirmation WHERE a.confirmer = 0";
    ...
    public function getHistoriqueListInterditLeveeCount()
    {
        try {
            $sql = "SELECT a.rowid, a.nom_complet, a.date_nais, CONCAT(u.prenom, ' ',u.nom), a.date_confirmation FROM alerts a INNER JOIN user u ON u.rowid = a.user_confirmation WHERE a.confirmer = 0";
            if ($_REQUEST['search']['value']!="") {
    ...
    public function getInfosalertByID($id)
    {
        try {
            $sql = "SELECT al.* , it.nom , it.prenom , it.date_nais as dateN , it.code as codeI , it.type_piece as tp , it.numero_piece as npiece 
                    FROM alerts as al 
                      INNER JOIN interdits as it ON al.id_interdit = it.rowid
                    WHERE al.rowid = ?";
            $user = $this->getConnexion()->prepare($sql);
            $user->execute([$id]);
    ...
    public function getParametrageInterdits()
    {
        try {
            $sql = "SELECT * FROM parametrage_interdits WHERE rowid = 1";
            $user = $this->getConnexion()->prepare($sql);
            $user->execute();
            return $user->fetchAll(PDO::FETCH_OBJ);
        } catch (PDOException $exception) {
    ...
    public function updateAlerteInstanceById($param)
    {
        try {
            $sql = "UPDATE alerts SET confirmer = ?, date_confirmation = now(), user_confirmation = ? WHERE rowid = ?";
            $user = $this->getConnexion()->prepare($sql);
    ...
    public function updateParametrageInterdits($param)
    {
        try {
            $champs = array_keys($param);
            $champs = array_map(function ($one){ return $one.' = ?'; }, $champs);
            $champs = implode(", ", $champs);

            $sql = "UPDATE parametrage_interdits SET $champs WHERE rowid = 1";
            $user = $this->getConnexion()->prepare($sql);
            return $user->execute(array_values($param));
        } catch (\PDOException $exception) {
            return false;
        }
    }

...
class JulaModel extends \app\core\BaseModel
{
    ...
    {
        try {
            $sql = "SELECT COUNT(idlotcarte_recu) as total FROM lotcarte_reception_jula ";
            $user = $this->getConnexion()->prepare($sql);
            $user->execute();
    ...
    {
        $data = null;
        $sql = "SELECT l.idlotcarte as rowid, l.num_reference, l.date_vente, l.num_debut, l.num_fin, l.montant, l.stock, a.label
                FROM lotcarte_jula as l
                INNER JOIN agence as a ON l.idagence = a.rowid";
        if(is_array($requestData)){
            $sql = "SELECT l.idlotcarte as rowid, l.num_reference, l.num_debut, l.num_fin, l.montant, l.stock_init, l.stock , l.carte_retour, a.rowid as idagence, a.label, l.user_add, l.date_add, l.idtypecarte
                    FROM lotcarte_jula as l 
                        INNER JOIN agence as a ON l.idagence = a.rowid ";
            $champs = array_keys($requestData);
            $champs = array_map(function ($one) {return $one . ' =:' . $one;}, $champs);
            ...
            $tabCol = ['l.num_reference', 'l.date_vente', 'l.num_debut', 'l.num_fin', 'l.montant', 'l.stock', 'a.label'];
            if (intval($_REQUEST['order'][0]['column']) < count($tabCol))
                $sql .= " ORDER BY " . $tabCol[$_REQUEST['order'][0]['column']] . " " . strtoupper($_REQUEST['order'][0]['dir']);
            $sql .= " LIMIT ".$_REQUEST['start']." ,".$_REQUEST['length'];
        }
    ...
    {
        try {
            $sql = "SELECT COUNT(idlotcarte) as total FROM lotcarte_jula ";
            $user = $this->getConnexion()->prepare($sql);
            $user->execute();
            $a = $user->fetchAll(PDO::FETCH_ASSOC);
            return $a[0]['total'];
    ...
    public function insertDistribution($data = [])
    {
        ...
        try {
            ...
            //echo $res = $agence->execute($data); die;
            //echo $this->updateReception(['stock' => (intval($dataTemp['stock']) - $data['stock'])], $data['idreception']);

...
class UtilisateurModel extends \app\core\BaseModel
{
    /**
 ...
 /**************Liste User*********/
    ...
    public function updatePasswordUtilisateurAfterFisterConnect($id, $password){

        $date_modification = date('Y-m-d H:i:s');
        try
        {
            $sql = "UPDATE user SET password=:pwd, connect=:conn, date_modification=:date_modification WHERE rowid=:id";
            $user = $this->getConnexion()->prepare($sql);
            $res = $user->execute(array("pwd"=>sha1('NUMH'.$password), "conn"=>1, "date_modification"=>$date_modification , "id"=>$id));
            $this->closeConnexion();
            if($res) return 1;
            else return -1;
        }

                ...
                <div class="box">
                    ...
                    <div class="box-body">
                         <div class="col-lg-offset-2 col-lg-8 col-md-offset-1 col-md-10 col-sm-12 col-xs-12">
                                <form class="form-horizontal" method="POST" action="<?= ROOT ?>reporting/dashboard">
                                	<fieldset style="display: block;" class="scheduler-border">
                                        <legend class="scheduler-border"><?= $data['lang']['tableau_bord_general']; ?></legend>
                                            <div class="form-group">
                                                <label for="region" class="col-sm-5 control-label">
                                                    <?= $data['lang']['produit'].' :' ; ?>
                                                </label>
                                                <div class="col-sm-7">
                                                    <select class="form-control select2" name="produit" style="width: 100%;" id="produit" required>
                                                        <option selected="selected" value=""><?= $data['lang']['select_service'] ; ?></option>
                                                        <?php foreach($data['service'] as $reg){ ?>
                                                            <option value="<?= $reg['rowid']; ?>"><?= $reg['label']; ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            </div>

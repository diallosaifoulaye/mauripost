...
<!--                                    <th>--><?//= $data['lang']['tel']; ?><!--</th>-->
                            ...
                            <input type="hidden" id="date1" value="<?php echo $data['datedeb'] ; ?>" />
                            <input type="hidden" id="date2" value="<?php echo $data['datefin']; ?>" />
                            <input type="hidden" id="numserie" value="<?php echo $data['numserie'] ; ?>" />
                        </div>
                    </div><!-- /.box-body -->
                    <div class="box-body margin-bottom">
                        ...
                        <div class="col-xs-6">
                            <form action="<?= ROOT ?>reporting/facturejour" method="post" name="form2" target="_blank">
                                <input type="hidden" name="date1" id="date1" value="<?php echo $data['datedeb'] ; ?>" />
                                <input type="hidden" name="date2" id="date2" value="<?php echo $data['datefin'] ; ?>" />
                                <input type="hidden" name="numserie" id="numserie" value="<?php echo $data['numserie'] ; ?>" />
...
<script type="text/javascript" language="javascript" >
    $(document).ready(function() {
        ...
        var dataTable = $('#user-grid').DataTable( {
            ...
            "serverSide": true,
            "ajax":{
                url : "<?= ROOT ?>reporting/processingByDateAndCompte/"+date1+"/"+date2+"/"+numserie, // json datasource
